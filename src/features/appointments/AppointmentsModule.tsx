...
const TODAY = new Date(); // usa la fecha real del sistema
...
/**
 * Mapea la respuesta del backend al tipo Appointment del frontend.
 * Ajusta los nombres de campo según lo que devuelva tu API.
 *
 ...
 *   cliente_telefono: "+57 310 123 4567",
 *   duracion_total: 60,
 *   servicios: [
 *     { serviceId: "s1", serviceName: "Masaje", employeeId: "e1", employeeName: "Ana", duration: 60, startTime: "10:00" }
 *   ]
 * }
 */
...
export function AppointmentsModule({ userRole }: AppointmentsModuleProps) {
  ...
  const handleAddService = () => {
    if (!currentService.serviceId || !currentService.employeeId) {
      toast.error("Selecciona un servicio y un empleado");
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`${API_BASE}/api/appointments/${appointmentToDelete}`, { method: "DELETE" });
  ...
  const handleCancelAppointment = async () => {
    ...
    try {
      await fetch(`${API_BASE}/api/appointments/${appointmentToCancel}/cancel`, { method: "PATCH" });
              ...
              <div key={apt.id} className="bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-all">
                <div className="grid grid-cols-1 lg:grid-cols-[80px_1.5fr_1.5fr_1.2fr_100px_1.8fr_120px_140px] gap-2 lg:gap-4 p-2.5 lg:p-4 items-start lg:items-center">
                  ...
                  <div className="flex items-center justify-start lg:justify-end gap-1.5">
                    <button onClick={() => setViewingAppointment(apt)} className="p-1.5 rounded-lg text-[#60A5FA] hover:bg-[#60A5FA]/10" title="Ver detalles">
                      <Edit className="w-4 h-4" />
                    </button>
                    {userRole === "admin" && (
                      <>
                        <button onClick={() => handleEdit(apt)} className="p-1.5 rounded-lg text-[#FBBF24] hover:bg-[#FBBF24]/10" title="Editar">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button
          ...
          {viewingAppointment && (
            <div className="space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {(["pending","completed","cancelled"] as const).map(s => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs transition-all ${viewingAppointment.status === s ? getStatusColor(s) + " ring-2 ring-offset-2" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                    >
                      {getStatusLabel(s)}
                    </button>
                  ))}
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-gray-200">
                {userRole === "admin" && (
                  <Button variant="outline" onClick={() => { setAppointmentToDelete(viewingAppointment.id); setDeleteDialogOpen(true); setViewingAppointment(null); }} className="border-[#F87171] text-[#F87171] hover:bg-[#F87171]/10">
                    <Trash2 className="w-4 h-4 mr-2" />Eliminar
                  </Button>
                )}
                <Button variant="outline" onClick={() => { handleEdit(viewingAppointment); setViewingAppointment(null); }} className="border-[#FBBF24] text-[#FBBF24] hover:bg-[#FBBF24]/10">
                  <Edit className="w-4 h-4 mr-2" />Editar
                </Button>
                <Button onClick={() => setViewingAppointment(null)} className="bg-gradient-to-r from-[#78D1BD] to-[#5FBFAA] text-white">
                  Cerrar
                </Button>
              </div>
            </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <Select value={currentService.serviceId} onValueChange={v => setCurrentService({ serviceId: v, employeeId: "" })}>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {getEmployeesByCategory(services.find(s => s.id === currentService.serviceId)?.category ?? "").map(e => (
              ...
              {selectedServices.length > 0 && (
                <div className="space-y-2 mt-3">
                  ...
                  {selectedServices.map((s, i) => {
                    const emp = employees.find(e => e.id === s.employeeId);
                    return (
                      <div key={i} className="flex items-center justify-between p-3 bg-white rounded-lg border-l-4" style={{ borderLeftColor: emp?.color ?? "#ccc" }}>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
              ...
              <Button onClick={handleCreateOrUpdate} className="bg-gradient-to-r from-[#78D1BD] to-[#5FBFAA] hover:from-[#6BCAB7] hover:to-[#4FB5A1] text-white">
